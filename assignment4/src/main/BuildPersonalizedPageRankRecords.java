...
public class BuildPersonalizedPageRankRecords extends Configured implements Tool {
  ...
  private static final String NODE_CNT_FIELD = "nodes";
  private static final String SRC_NODES_FIELD = "src";  
  //TODO will use this for page rank array
  ...
  private static class MyMapper extends Mapper<LongWritable, Text, IntWritable, PageRankNode> {
    ...
    private static final IntWritable source = new IntWritable();
    //StringWritable 
    //TODO change to array above for multi-source 
    ...
    public void setup(Mapper<LongWritable, Text, IntWritable, PageRankNode>.Context context) {
      ...
      source.set(Integer.parseInt(srcs[0]));
    ...
    public void map(LongWritable key, Text t, Context context) throws IOException,
      ...
      if (node.getNodeId() == source.get()) {
        // Source nodes are initialized to log(1) = 0
        node.setPageRank(0);

      }
      else {
        // Non-source nodes are initialized to log(0)
        node.setPageRank(Float.NEGATIVE_INFINITY);
      }
      
      //LOG.info("nodeid: "+ node.getNodeId());
      //LOG.info("page rank: " + node.getPageRank());
  ...
  public int run(String[] args) throws Exception {
    ...
    LOG.info(" - numNodes: " + n);
    LOG.info(" - sourceNodes: " + src); 

    Configuration conf = getConf();
    conf.setInt(NODE_CNT_FIELD, n);
    conf.setInt("mapred.min.split.size", 1024 * 1024 * 1024);
    conf.setStrings(SRC_NODES_FIELD, src);  
