...
public class RunPersonalizedPageRankBasic extends Configured implements Tool {
  ...
  private static class MapPageRankMassDistributionClass extends
    ...
    public void setup(Context context) throws IOException {
      ...
      sources = conf.getStrings("SourceNodes");
      source = Integer.parseInt(sources[0]);
    }
    ...
    //All mass lost in the dangling nodes are put back into the source node; 
    //this is unlike in ordinary PageRank, where the missing mass is evenly distributed across all nodes.
    
    public void map(IntWritable nid, PageRankNode node, Context context)
        throws IOException, InterruptedException {
      float p = node.getPageRank();
      
      //Source nodes get all dangling mass and all jumps as contributions.
      ...
      //Non-source nodes get no jumps and no dangling mass as contributions. 
      else {
        float link = (float) Math.log(1.0f - ALPHA)
            + p;
  ...
  public int run(String[] args) throws Exception {
    ...
    // Iterate PageRank.
    for (int i = s; i < e; i++) {
      iteratePageRank(i, i + 1, basePath, n, useCombiner, useInmapCombiner, src);
    }
  ...
  private void iteratePageRank(int i, int j, String basePath, int numNodes,
      boolean useCombiner, boolean useInMapperCombiner, String sourceNodes) throws Exception {
    ...
    phase2(i, j, missing, basePath, numNodes, sourceNodes); //TODO pass the source nodes here (only phase 2 needs it)
  }
